...
description: 
published: true
date: 2023-11-07T09:34:10.032Z
tags: 
editor: ckeditor
dateCreated: 2023-11-07T09:20:58.768Z
-->
...
Для реквизита **Выбор из списка** необходимо задать список значений во вкладке **Формирование списка значений для выбора на  КПК**

Вкладка **Формирование списка значений для выбора на  КПК**  - код на языке 1С формирующий данные для заполнения реквизита **Список значений**.

Вкладка **Обработка значения реквизита в 1С**  - код на языке 1С который будет срабатывать при создании документа и обрабатывать выгруженные с КПК значение реквизита.
...
**Договор** \- СправочникОбъект. ДоговорыКонтрагентов если дополнительный реквизит привязан к новому клиенту.
...
Запрещено переопределять следующие переменные: **Стр**, **Табл**.

**обработка.ПолучитьКонтрагентаПоКоду(ОбщийКод, Контрагент) -**  По идентификатору клиента Моби-С возвращает ссылку на контрагента. Доступно только в "**Обработка значения реквизита в 1С**".

**обработка.ПолучитьНоменклатуру(Код, Номенклатура) -** По идентификатору товара Моби-С возвращает ссылку на номенклатуру в 1С. Доступно только в "**Обработка значения реквизита в 1С**".

**обработка.ПолучитьСправочникСсылкаПоГУИД(ГУИД, имяСправочника) -** Функция возвращает СправочникСсылка указанного в имяСправочника типа, по переданному ГУИДу.
...
**обработка.ПолучитьДокументСсылкаПоГУИД(ГУИД, имяДокумента) -** Функция возвращает ДокументСсылка указанного в имяДокумента типа, по переданному ГУИДу.
...
> **Методы получения кодов Контрагент+Договор, Номенклатура, больше не используются,**
> 
> **требуемые коды добавлены в списки значений – Клиенты, Номенклатуры.**
...
**Договоры** \- Массив договоров доступных на КПК при выборе клиента - массив структур со следующими свойствами:

-   Договор - ссылка на соглашение с клиентом;
-   ДоговорИД - идентификатор соглашения на КПК;
-   Организация - ссылка на организация из договора
-   Контрагент - ссылка на контрагента.

**Клиенты** \- массив структур со следующими свойствами:

-   ГоловнойКонтрагент - ссылка на контрагента;
-   Контрагент - ссылка на контрагента;
-   Родитель - ссылка на договор;
